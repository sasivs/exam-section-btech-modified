...
from ADAUGDB.user_access_test import is_Hod, co_ordinator_assignment_access
...
def faculty_Coordinator_Status(request):
    if(request.user.groups.filter(name__in=['Superintendent', 'Associate-Dean-Academics', 'Associate-Dean-Exams']).exists()):
        user = request.user
