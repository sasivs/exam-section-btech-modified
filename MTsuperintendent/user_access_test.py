...
def roll_list_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Superintendent' in groups or 'Co-ordinator' in groups or 'HOD' in groups  or 'Associate-Dean' in groups:
        return True
    return  False

def subject_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Superintendent' in groups or 'Co-ordinator' in groups  :
        return True
    return False
...
def registration_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Co-ordinator' in groups or 'Superintendent' in groups or 'HOD' in groups or 'Associate-Dean' in groups:
        return True
    return False 
...
def grades_threshold_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Faculty' in groups or 'Co-ordinator' in groups or 'HOD' in groups or 'Superintendent' in groups or 'Associate-Dean' in groups:
        return True
    return False 
...
def attendance_shortage_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Superintendent' in groups or 'Co-ordinator' in groups or 'HOD' in groups or 'Faculty' in groups or 'Associate-Dean' in groups:
        return True
    return  False
...
def grades_home_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Superintendent' in groups or 'Co-ordinator' in groups or 'HOD' in groups or 'ExamStaff' in groups  or\
        'Faculty' in groups:
        return True
    return  False
...
def grades_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Superintendent' in groups or 'Co-ordinator' in groups or 'HOD' in groups or 'Faculty' in groups \
        or 'Associate-Dean' in groups:
        return True
    return  False
...
def not_promoted_status_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'HOD' in groups or 'Superintendent' in groups or 'Co-ordinator' in groups :
        return True
    return  False
...
def sample_regno_sheet_access(user):
    groups = user.groups.all().values_list('name', flat=True)
    if 'Co-ordinator' in groups or 'Faculty' in groups or 'Superintendent' in groups:
        return True
    return  False
