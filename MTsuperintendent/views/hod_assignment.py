...
from MTsuperintendent.user_access_test import is_Superintendent, hod_assignment_status_access
from django.shortcuts import render
from django.utils import timezone
from MTsuperintendent.models import MTHOD
from MTsuperintendent.forms import HODAssignmentForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if form.cleaned_data.get('dept') and form.cleaned_data.get('hod') and form.cleaned_data.get('user') and 'submit-form' in request.POST.keys():
                initial_hod = MTHOD.objects.filter(RevokeDate__isnull=True, Dept=form.cleaned_data.get('dept')).first()
                if initial_hod:
