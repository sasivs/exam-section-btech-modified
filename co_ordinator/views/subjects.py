...
def subject_upload(request):
            ...
            if request.POST.get('upload_file_submit'):
                if(form.cleaned_data['regID']!='--Choose Event--'):
                    ...
                    errorDataset.headers = ['SubCode', 'SubName', 'BYear', 'BSem', 'Dept','OfferedYear', 'Regulation',\
                        'Creditable', 'Credits','Type','Category', 'OfferedBy', 'DistributionRatio']
    ...
    return (render(request, 'co_ordinator/BTSubjectsUpload.html', {'form':form}))
...
def subject_upload_error_handler(request):
    ...
    return(render(request, 'co_ordinator/BTSubjectsUploadErrorHandler.html',{'form':form}))
...
def subject_delete_handler(request,event):
    if(request.method=='POST'):
        ...
        form = SubjectDeletionForm()
    return render(request, 'co_ordinator/BTSubjectsDeletionHandler.html',{'form':form })
        ...
        if(form.is_valid()):
            ...
            if(form.cleaned_data['regID']!='--Choose Event--'):
                ...
                for sub in subjects:
                   ...
                   s.save() 
                return render(request, 'co_ordinator/BTSubjectFinalizeSuccess.html')
        else:
            print('Invalid')
    else:
        form = SubjectFinalizeEventForm()
    return render(request, 'co_ordinator/BTSubjectFinalize.html',{'form':form})
