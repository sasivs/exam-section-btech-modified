        ...
        if form.is_valid():
            if form.cleaned_data.get('dept') and form.cleaned_data.get('hod') and form.cleaned_data.get('user') and 'submit-form' in request.POST.keys():
                ...
                if initial_hod:
                    ...
                    new_hod.save()
                    msg = 'Hod assignment is done successfully'
                return render(request, 'BTsuperintendent/HodAssignment.html', {'form':form, 'msg':msg})
            else:
                if 'dept' in request.POST.keys():
                    initial_hod = BTHOD.objects.filter(Dept=request.POST.get('dept'), RevokeDate__isnull=True).first()
                    if initial_hod:
                        return render(request, 'BTsuperintendent/HodAssignment.html', {'form':form, 'initial_hod':initial_hod.Faculty.id, 'initial_user':initial_hod.User.id})
    else:
        form = HODAssignmentForm()
    return render(request, 'BTsuperintendent/HodAssignment.html', {'form':form})
