...
def roll_list_script(kwargs):
    ...
    for event in events:
        ...
        if event.Mode == 'R':
            ...
            dropped_courses_students = BTDroppedRegularCourses.objects.filter(Registered=False, subject_RegEventId__BYear=event.BYear, subject__RegEventId__Regulation=event.Regulation, RegEventId__Dept=event.Dept).\
                exclude(subject__RegEventId__AYear=event.AYear).distinct('student__RegNo').order_by('student__RegNo')
