...
def dropped_regular_registrations(request):
    ...
    if(request.method == 'POST'):
        ...
        if 'RegEvent' in request.POST and 'RegNo' in request.POST and not 'Submit' in request.POST:
            roll = BTRollLists_Staging.objects.filter(id=request.POST['RegNo']).first()
            studentRegistrations = BTStudentRegistrations_Staging.objects.filter(student__student=roll.student, RegEventId__AYear=event.AYear, \
                    ...
                    if(sub[6]=='R'): #Handling Regular Subjects
                        if(form.cleaned_data['Check'+str(sub[9])] == False):
                            ...
                            if len(reg) != 0:
                                ...
                                new_dropped_course = BTDroppedRegularCourses(student=roll.student, subject_id=sub[9], RegEventId_id=reg.RegEventId, Registered=False)
