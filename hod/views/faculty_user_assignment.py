...
def faculty_Coordinator(request):
    ...
    if(request.method == 'POST'):
        ...
        form = CoordinatorAssignmentForm()
...
#     #print(User.objects.all().values())
#     already_assigned = Faculty_user.objects.filter(RevokeDate__isnull=True)
#     already_assigned2 = Coordinator.objects.filter(RevokeDate__isnull=True)
#     already_assigned = already_assigned.union(already_assigned2)
#     users = User.objects.all().exclude(id__in=already_assigned.values_list('User_id', flat=True))
#     user_assigned = Coordinator.objects.filter(Faculty_id = pk,RevokeDate__isnull=True)
#     assigned_user =''
#     if user_assigned :
#         assigned_user = user_assigned[0].User_id
#         assign_user = User.objects.filter(id=user_assigned[0].User_id)
...
#             facuser = Coordinator.objects.filter(Faculty_id= pk,RevokeDate__isnull=True)
#             if facuser:
#                 facuser.update(User_id =  request.POST.get('fac_Coordinator'))
#             else:
#                facuser= Coordinator(User_id =  request.POST.get('fac_Coordinator'),Faculty_id = pk)
#                facuser.save()
...
#     faculty = FacultyInfo.objects.get(id=pk)
#     User = get_user_model()
#     fac = Coordinator.objects.filter(Faculty_id=pk,RevokeDate__isnull=True)
#     if fac:
#         fac.update(RevokeDate =timezone.now())
#     return redirect('FacultyCoordinatorAssignment')
