...
class HODAssignmentForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['hod'] = forms.CharField(label='HOD', widget=forms.Select(choices=HOD_CHOICES, attrs={{'onchange':"submit()"}}))
        self.fields['user'] = forms.CharField(label='User', widget=forms.Select(choices=USER_CHOICES, attrs={{'onchange':"submit()"}}))
        if self.data.get('dept'):
            ...
            initial_hod = HOD.objects.filter(Dept=self.data.get('hod'), RevokeDate__isnull=True).first()
            if initial_hod:
