...
from .models import NotPromoted, ProgrammeModel, RegistrationStatus, StudentMakeups, StudentRegistrations, StudentInfo, Subjects,\
     Subjects_Staging, Regulation, DroppedRegularCourses, StudentRegistrations_Staging
...
class MakeupRegistrationsForm(forms.Form):
    def __init__(self, *args,**kwargs):
        ...
        if 'RegEvent' in self.data and self.data['RegEvent']!= '-- Select Registration Event --':
            ...
            regulation = int(strs[5])
            mode = strs[6]
            studentMakeupRolls = StudentMakeups.objects.filter(Dept=dept, BYear=byear).values('RegNo').distinct()
